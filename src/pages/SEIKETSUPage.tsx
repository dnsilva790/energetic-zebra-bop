...
const SEIKETSUPage = () => {
  ...
  const handleReprogramSelected = useCallback(async () => {
    ...
    const tomorrow = addDays(utcToZonedTime(new Date(), BRASILIA_TIMEZONE), 1); // Usar utcToZonedTime diretamente
    const tomorrowFormatted = formatInTimeZone(tomorrow, BRASILIA_TIMEZONE, "yyyy-MM-dd"); // Usar formatInTimeZone diretamente
    ...
    for (const taskId of selectedPendingTasks) {
      ...
      if (taskToReprogram) {
        const success = await handleApiCall(
          () => updateTaskDueDate(taskId, tomorrowFormatted),
